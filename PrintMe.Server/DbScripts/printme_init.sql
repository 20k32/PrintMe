...
$$
    BEGIN
        IF
            EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'printme_owner') THEN
            RAISE NOTICE 'Role printme_owner exists, dropping owned objects.';
            DROP
                OWNED BY printme_owner;
        ELSE
...
CREATE DATABASE printme_db
    WITH
    OWNER = printme_owner ENCODING = 'UTF8' LOCALE_PROVIDER = 'libc' TABLESPACE = pg_default CONNECTION
